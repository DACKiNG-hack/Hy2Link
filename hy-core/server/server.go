...
type Server interface {
	Serve() error
	Close() error
	HandleConn(conn *quic.Conn) // 新增：外部 accept 后交给 hy-core 处理
}
...
func NewServer(config *Config) (Server, error) {
	...
	if config.Transport != nil {
		// 模式 A：外部传入共享 transport，由 hy-core 自己 Listen
		tr = config.Transport
		var err error
		...
		// 模式 B：自己创建 transport + listener（原有行为）
		tr = &quic.Transport{
			Conn:       config.Conn,
		...
		// 模式 C（新增）：无 Conn 也无 Transport
		// listener 保持 nil，由外部 accept 循环通过 HandleConn 驱动
		// 用于单端口 ALPN 分流方案
	}
...
func (s *serverImpl) Serve() error {
	if s.listener == nil {
		return errors.New("Serve: no listener available, use HandleConn instead")
	}
	for {
...
// HandleConn 处理一个已经 accept 的 QUIC 连接
// 由外部 accept 循环根据 ALPN 分发
func (s *serverImpl) HandleConn(conn *quic.Conn) {
	go s.handleClient(conn)
}
...
func (s *serverImpl) Close() error {
	...
	if s.ownTransport {
		...
		if s.config.Conn != nil {
			errs = append(errs, s.config.Conn.Close())
		}
	}
	if s.config.Cleanup != nil {
		errs = append(errs, s.config.Cleanup.Close())
	}
